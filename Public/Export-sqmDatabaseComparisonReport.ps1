<#
.SYNOPSIS
    Builds a self-contained HTML report showing, for every table between a source and destination
    database, whether the transfer is complete or still missing/mismatched.

.DESCRIPTION
    Consumes the output of Compare-sqmDatabaseRowCount and renders it as one table: source rows,
    destination rows, difference and a status badge (Vollstaendig / Abweichung / Fehlt auf
    Ziel / Fehlt auf Quelle). Intended as the single consolidated view to run instead of opening a
    separate per-call report for every table when transferring a large table set one call at a
    time (e.g. from the GUI's table grid, or several Invoke-sqmTableTransfer calls with -NoReport) -
    that pattern otherwise produces one report file per call with no overview of what's actually
    still outstanding across the whole set.

.PARAMETER Source
    Source SQL Server instance (for the report header only).

.PARAMETER SourceDatabase
    Source database name (for the report header only).

.PARAMETER Destination
    Target SQL Server instance (for the report header only).

.PARAMETER DestinationDatabase
    Target database name (for the report header only).

.PARAMETER Comparison
    Comparison objects as returned by Compare-sqmDatabaseRowCount
    (Table/SourceRows/DestinationRows/Difference/Status/Verified).

.PARAMETER FilePath
    Path to write the HTML report to.

.PARAMETER Title
    Report heading. Default: 'sqmDataTransfer - Gesamtvergleich'.

.PARAMETER NoOpen
    Do not automatically open the report after writing it (default: opens it, same convention as
    Export-sqmTransferReport).

.PARAMETER PassThru
    Also return the generated HTML as a string.

.EXAMPLE
    $cmp = Compare-sqmDatabaseRowCount -Source SQL01 -SourceDatabase Sales -Destination SQL02 -DestinationDatabase Sales -VerifyMismatches
    Export-sqmDatabaseComparisonReport -Source SQL01 -SourceDatabase Sales -Destination SQL02 -DestinationDatabase Sales -Comparison $cmp -FilePath C:\Temp\Gesamtvergleich.html

.NOTES
    Prerequisites: none beyond the module itself - pure string/HTML generation.
    Companion to Export-sqmTransferReport (per-run report); this one is database-wide and
    independent of any single Invoke-sqmTableTransfer call.
#>
function Export-sqmDatabaseComparisonReport
{
	[CmdletBinding()]
	[OutputType([void], [string])]
	param (
		[Parameter(Mandatory = $true)]
		[string]$Source,
		[Parameter(Mandatory = $true)]
		[string]$SourceDatabase,
		[Parameter(Mandatory = $true)]
		[string]$Destination,
		[Parameter(Mandatory = $true)]
		[string]$DestinationDatabase,
		[Parameter(Mandatory = $true)]
		[AllowEmptyCollection()]
		[PSCustomObject[]]$Comparison,
		[Parameter(Mandatory = $true)]
		[string]$FilePath,
		[Parameter(Mandatory = $false)]
		[string]$Title = 'sqmDataTransfer - Gesamtvergleich',
		[Parameter(Mandatory = $false)]
		[switch]$NoOpen,
		[Parameter(Mandatory = $false)]
		[switch]$PassThru
	)

	function Escape-Html([string]$s)
	{
		if ($null -eq $s) { return '' }
		[System.Net.WebUtility]::HtmlEncode($s)
	}

	$sorted = @($Comparison | Sort-Object Table)
	$totalTables = $sorted.Count
	$matchCount = @($sorted | Where-Object Status -eq 'Match').Count
	$mismatchCount = @($sorted | Where-Object Status -eq 'Mismatch').Count
	$missingDstCount = @($sorted | Where-Object Status -eq 'MissingOnDestination').Count
	$missingSrcCount = @($sorted | Where-Object Status -eq 'MissingOnSource').Count
	$openCount = $totalTables - $matchCount

	[Int64]$totalRowsTransferred = 0
	foreach ($c in $sorted)
	{
		if ($null -ne $c.DestinationRows) { $totalRowsTransferred += [Int64]$c.DestinationRows }
	}
	$totalRowsTransferredText = '{0:N0}' -f $totalRowsTransferred

	$rowsHtml = New-Object System.Text.StringBuilder
	if ($totalTables -eq 0)
	{
		[void]$rowsHtml.Append('<tr><td colspan="5" class="empty">Keine Tabellen gefunden.</td></tr>')
	}
	else
	{
		foreach ($c in $sorted)
		{
			switch ($c.Status)
			{
				'Match' { $badge = '<span class="badge ok">Vollstaendig</span>' }
				'Mismatch' { $badge = '<span class="badge warn">Abweichung</span>' }
				'MissingOnDestination' { $badge = '<span class="badge err">Fehlt auf Ziel</span>' }
				'MissingOnSource' { $badge = '<span class="badge err">Fehlt auf Quelle</span>' }
				default { $badge = "<span class=`"badge warn`">$(Escape-Html $c.Status)</span>" }
			}
			if ($c.Verified) { $badge += ' <span class="verified">geprueft</span>' }
			$srcRows = if ($null -ne $c.SourceRows) { "{0:N0}" -f [int64]$c.SourceRows } else { '-' }
			$dstRows = if ($null -ne $c.DestinationRows) { "{0:N0}" -f [int64]$c.DestinationRows } else { '-' }
			$diffText = if ($null -ne $c.Difference) { "{0:N0}" -f [int64]$c.Difference } else { '-' }
			[void]$rowsHtml.Append("<tr><td>$(Escape-Html $c.Table)</td><td class=`"num`">$srcRows</td>" + `
					"<td class=`"num`">$dstRows</td><td class=`"num`">$diffText</td><td>$badge</td></tr>")
		}
	}

	$generated = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'

	$html = @"
<!DOCTYPE html>
<html lang="de">
<head>
<meta charset="UTF-8">
<title>$(Escape-Html $Title)</title>
<style>
:root{--bg:#faf9f5;--card:#fff;--ink:#1f1e1b;--mut:#6b6a64;--bd:#e3e1d8;--blue:#378ADD;--green:#1D9E75;--amber:#BA7517;--red:#E24B4A}
@media(prefers-color-scheme:dark){:root{--bg:#1b1b19;--card:#252523;--ink:#ECEAE2;--mut:#9c9b93;--bd:#3a3a36}}
body{margin:0;background:var(--bg);color:var(--ink);font-family:Segoe UI,system-ui,sans-serif;font-size:15px;line-height:1.5}
.wrap{max-width:980px;margin:0 auto;padding:24px}
h1{font-size:20px;font-weight:600;margin:0 0 4px}
h2{font-size:16px;font-weight:600;margin:28px 0 10px}
.sub{color:var(--mut);font-size:13px;margin:0 0 18px}
.chips{display:flex;gap:10px;flex-wrap:wrap;margin:14px 0 4px}
.chip{border:1px solid var(--bd);border-radius:8px;padding:8px 14px;font-size:13px;background:var(--card);color:var(--mut);display:flex;align-items:center;gap:8px}
.chip b{color:var(--ink);font-size:15px}
.chip.ok{border-color:var(--green)} .chip.ok b{color:var(--green)}
.chip.warn{border-color:var(--amber)} .chip.warn b{color:var(--amber)}
.chip.err{border-color:var(--red)} .chip.err b{color:var(--red)}
table{width:100%;border-collapse:collapse;font-size:13.5px;background:var(--card);border:1px solid var(--bd);border-radius:8px;overflow:hidden}
th{background:var(--bd);color:var(--ink);padding:8px 12px;text-align:left;font-weight:600;font-size:12px;text-transform:uppercase;letter-spacing:.03em}
td{padding:7px 12px;border-top:1px solid var(--bd);vertical-align:top}
td.num{text-align:right;font-variant-numeric:tabular-nums}
tr:hover td{background:rgba(128,128,128,.06)}
.empty{color:var(--mut);font-style:italic}
.badge{display:inline-block;padding:2px 9px;border-radius:20px;font-size:11.5px;font-weight:600}
.badge.ok{background:rgba(29,158,117,.15);color:var(--green)}
.badge.warn{background:rgba(186,117,23,.15);color:var(--amber)}
.badge.err{background:rgba(226,75,74,.15);color:var(--red)}
.verified{color:var(--mut);font-size:11px}
footer{margin-top:28px;color:var(--mut);font-size:12px}
</style>
</head>
<body><div class="wrap">
<h1>$(Escape-Html $Title)</h1>
<p class="sub">$(Escape-Html $Source).$(Escape-Html $SourceDatabase) &rarr; $(Escape-Html $Destination).$(Escape-Html $DestinationDatabase) &nbsp;|&nbsp; erzeugt $generated</p>

<div class="chips">
    <div class="chip"><b>$totalTables</b> Tabelle(n) gesamt</div>
    <div class="chip"><b>$totalRowsTransferredText</b> Zeilen gesamt übertragen</div>
    <div class="chip ok"><b>$matchCount</b> vollstaendig</div>
    <div class="chip $(if ($openCount -gt 0) { 'err' } else { 'ok' })"><b>$openCount</b> noch offen</div>
    <div class="chip $(if ($mismatchCount -gt 0) { 'warn' } else { 'ok' })"><b>$mismatchCount</b> Abweichung(en)</div>
    <div class="chip $(if ($missingDstCount -gt 0) { 'err' } else { 'ok' })"><b>$missingDstCount</b> fehlt auf Ziel</div>
    <div class="chip $(if ($missingSrcCount -gt 0) { 'warn' } else { 'ok' })"><b>$missingSrcCount</b> fehlt auf Quelle</div>
</div>

<h2>Tabellen (Quelle vs. Ziel)</h2>
<table>
<thead><tr><th>Tabelle</th><th>Quelle (Zeilen)</th><th>Ziel (Zeilen)</th><th>Differenz</th><th>Status</th></tr></thead>
<tbody>
$($rowsHtml.ToString())
</tbody>
</table>

<footer>sqmDataTransfer $((Get-sqmTransferConfig -Key 'ModuleVersion'))</footer>
</div></body>
</html>
"@

	try
	{
		$dir = Split-Path $FilePath -Parent
		if ($dir -and -not (Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }
		[System.IO.File]::WriteAllText($FilePath, $html, (New-Object System.Text.UTF8Encoding($false)))
		Write-sqmTransferLog -Message (Get-sqmTransferString -Key 'ExportReport.Written' -FormatArgs @($FilePath)) -FunctionName $MyInvocation.MyCommand.Name -Level 'INFO'
		Invoke-sqmTransferOpenReport -HtmlFile $FilePath -NoOpen:$NoOpen
	}
	catch
	{
		Write-Warning (Get-sqmTransferString -Key 'ExportReport.WriteFailedWarning' -FormatArgs @($FilePath, $_.Exception.Message))
		Write-sqmTransferLog -Message (Get-sqmTransferString -Key 'ExportReport.WriteFailedLog' -FormatArgs @($FilePath, $_.Exception.Message)) -FunctionName $MyInvocation.MyCommand.Name -Level 'ERROR'
	}

	if ($PassThru) { return $html }
}
